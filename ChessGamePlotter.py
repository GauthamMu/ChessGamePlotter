...
def printChessBoard():
    ...
    if piecesBlackCaptured:
        print(" ".join(piecesBlackCaptured)) # Prints piecesBlackCaptured, with a space as seperator, if it's not empty
    ...
    if piecesWhiteCaptured:
        print(" ".join(piecesWhiteCaptured)) # Prints piecesWhiteCaptured, with a space as seperator, if it's not empty

# To validate the chess square part of the input
possibleChessSquareCharacters = ["a", "b", "c", "d", "e", "f", "g", "h", "1", "2", "3", "4", "5", "6", "7", "8", "K", "Q", "B", "N", "R"] # Includes piece letters
def chessSquareValidation(square):
    ...
    for i in square:
        if i not in possibleChessSquareCharacters: # Returns early if the chess square contains characters outside of a-h, 1-8
    ...
    if file.isalpha() and rank.isdigit(): # Returns this if chess square is letter and digit (normal)
        ...
        if ord(file) == ord(rank) + 1 or 1 + ord(file) == ord(rank): # Detects if the files are adjacent
...
def pawnMoveValidation(move):
    if len(move) == 2:
        ...
        if typeOfSquare == "invalid pawn capture": # Gets called if square coordinates are files which are not next to each other
            ...
            if currentTurn == "w": # Current player is white
                if move[1] in ["1", "2"]: # Fires when white attempts to move pawn backwards or sideways with a pawn
                    print("You can't move your pawn backwards or sideways. Use algebraic notation.")
                    return "invalid"
                else:
                    if chessBoard[8-int(move[1])][fileToIndex[move[0]]] in [wSqr, bSqr]: # Not a regular capture
                        if chessBoard[9-int(move[1])][fileToIndex[move[0]]] == wp: # Checking if the square in front of the desired square is a white pawn, which it is unless initial two-square advance
                            executeMove([9-int(move[1]), fileToIndex[move[0]]], [8-int(move[1]), fileToIndex[move[0]]]) # Using the new executeMove function
                        elif move[1] == "4" and chessBoard[6][fileToIndex[move[0]]] == wp and chessBoard[5][fileToIndex[move[0]]] in [wSqr, bSqr]: # Checking if initial two-square advance
                            executeMove([6, fileToIndex[move[0]]], [4, fileToIndex[move[0]]]) # Using the new executeMove function
                    else: # A regular capture
                        pass 
            else: # Current player is black
                if move[1] in ["7", "8"]: # Fires when black attempts to move pawn backwards or sideways with a pawn
                    print("You can't move your pawn backwards or sideways. Use algebraic notation.")
                    return "invalid"
                else:
                    if chessBoard[8-int(move[1])][fileToIndex[move[0]]] in [wSqr, bSqr]: # Not a regular capture
                        if chessBoard[7-int(move[1])][fileToIndex[move[0]]] == bp: # Checking if the square in front of the desired square is a black pawn, which it is unless initial two-square advance
                            chessBoard[8-int(move[1])][fileToIndex[move[0]]] = chessBoard[7-int(move[1])][fileToIndex[move[0]]] # Moving pawn to the desired square
                            clearSquare(7-int(move[1]), fileToIndex[move[0]]) # Clears initial pawn position
                        elif move[1] == "5" and chessBoard[1][fileToIndex[move[0]]] == bp  and chessBoard[2][fileToIndex[move[0]]] in [wSqr, bSqr]: # Checking if initial two-square advance
                            chessBoard[3][fileToIndex[move[0]]] = chessBoard[1][fileToIndex[move[0]]] # Moving pawn to the desired square
                            clearSquare(1, fileToIndex[move[0]]) # Clears initial pawn position
            ...
            if currentTurn == "w": # Current player is white
                ...
                if len(possibleOriginSquares) == 1:
                    executeMove(possibleOriginSquares[0], [possibleOriginSquares[0][0]-1, fileToIndex[move[1]]])
                elif len(possibleOriginSquares) == 2:
                    print("I didn't implement being able to choose between two different pawns yet")
            else: # Current player is black
        ...
        if move[0] == "x": # Pawn capture
            ...
            if typeOfSquare == "normal chess square":
                if currentTurn == "w": # Current player is white
                    if move[1] in ["1", "2"]: # Fires when white attempts to capture piece backwards or sideways with a pawn
                        print("You can't capture with your pawn backwards or sideways. Use algebraic notation.")
                        return "invalid"
                    else:
                        if chessBoard[8-int(move[2])][fileToIndex[move[1]]] in blackPieces: # A regular capture
                            if move[1] != "a":
                                if chessBoard[9-int(move[2])][fileToIndex[move[1]]-1] == wp:
                                    possibleOriginSquares.append([9-int(move[2]), fileToIndex[move[1]]-1])
                            if move[1] != "h":
                                if chessBoard[9-int(move[2])][fileToIndex[move[1]]+1] == wp:
                                    possibleOriginSquares.append([9-int(move[2]), fileToIndex[move[1]]+1])
                            if len(possibleOriginSquares) == 1:
                                executeMove(possibleOriginSquares[0], [8-int(move[2]), fileToIndex[move[1]]])
                            elif len(possibleOriginSquares) == 2:
                                print("I didn't implement being able to choose between two different pawns yet")
                        elif chessBoard[8-int(move[2])][fileToIndex[move[1]]] in emptySqrs: # En passant capture
                            pass
                        else:
                            print("You can't eat your own pieces with your own pawn.")
                            return "invalid"
                else: # Current player is black
                    if move[1] in ["7", "8"]: # Fires when black attempts to capture piece backwards or sideways with a pawn
                        print("You can't capture with your pawn backwards or sideways. Use algebraic notation.")
                        return "invalid"
                    else:
                        if chessBoard[8-int(move[2])][fileToIndex[move[1]]] in whitePieces: # A regular capture
                            if move[1] != "a":
                                if chessBoard[7-int(move[2])][fileToIndex[move[1]]-1] == bp:
                                    possibleOriginSquares.append([7-int(move[2]), fileToIndex[move[1]]-1])
                            if move[1] != "h":
                                if chessBoard[7-int(move[2])][fileToIndex[move[1]]+1] == bp:
                                    possibleOriginSquares.append([7-int(move[2]), fileToIndex[move[1]]+1])
                            if len(possibleOriginSquares) == 1:
                                ...
                                print("I didn't implement being able to choose between two different pawns yet")
                        elif chessBoard[8-int(move[1])][fileToIndex[move[0]]] in emptySqrs: # En passant capture
                            pass
                        else:
                            print("You can't eat your own pieces with your own pawn.")
                            return "invalid"
...
def aM2bM(move, fileMod = 0, rankMod = 0): # Function to convert algebraic notation to "board notation" (my chess game's internally used notation, to access the pieces)
    if len(move) != 2:
...
def knightTheoreticalMoves(move):
    move = move[1:]
    allMoves = [aM2bM(move, 1, 2), aM2bM(move, 2, 1), aM2bM(move, -1, 2), aM2bM(move, -2, 1), aM2bM(move, 1, -2), aM2bM(move, 2, -1), aM2bM(move, -1, -2), aM2bM(move, -2, -1)]
    theoreticalMoves = []
...
def knightPossibleMoves(move):
    kTM = knightTheoreticalMoves(move) # Knight's Theoretical Moves (taking chess board borders into consideration, but not locations of own knights)
    kPM = [] # List for Knight's Possible Moves (taking locations of own knights into consideration)
    ...
    if len(move) == 3:
        ...
        if typeOfSquare == "normal chess square":
            ...
            if len(possibleOriginSquares) == 1:
                ...
                print("I haven't implemented selecting between multiple Knights yet (knightMoveValidation, line 295)")
    elif len(move) == 4:
        if move[1] == "x":
            ...
            if typeOfSquare == "normal chess square":
                ...
                if len(possibleOriginSquares) == 1:
                    ...
                    print("I haven't implemented selecting between multiple Knights yet (knightMoveValidation, line 305)")
    elif len(move) == 5:
        pass
    ...
    if len(originSqr) == 2 and len(destinationSqr) == 2:
        if chessBoard[destinationSqr[0]][destinationSqr[1]] not in emptySqrs:
            if chessBoard[destinationSqr[0]][destinationSqr[1]] in whitePieces:
                piecesBlackCaptured.append(chessBoard[destinationSqr[0]][destinationSqr[1]])
            else:
                piecesWhiteCaptured.append(chessBoard[destinationSqr[0]][destinationSqr[1]])
        chessBoard[destinationSqr[0]][destinationSqr[1]] = chessBoard[originSqr[0]][originSqr[1]] # Moving pawn to the desired square
...
while True: # Loop for chess game
    currentTurn, currentTurnL = switchCurrentPlayer(currentTurn) # Switches current turn
    move = input(f"Enter {currentTurnL} move: ").strip() # Strip to remove leading and trailing whitespace
